...
class SeerrProxyView(HomeAssistantView):
    ...
    async def _proxy(self, request: web.Request, path: str, method: str) -> web.Response:
        ...
        _LOGGER.debug("Seerr proxy %s %s", method, target)
        ...
        try:
            ...
            async with api._session.request(method, target, **kwargs) as resp:
                ...
                if resp.status >= 400:
                    ...
                    _LOGGER.debug("Seerr proxy %s %s -> HTTP %s", method, target, resp.status)
                    # Fire refresh event when a request is successfully submitted
                    if method == "POST" and path.rstrip("/") == "request" and resp.status < 300:
